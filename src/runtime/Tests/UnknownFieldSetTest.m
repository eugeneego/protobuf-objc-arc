...
- (void)setUp
{
  ...
  self.allFieldsData = [_allFields data];
  self.emptyMessage = [TestEmptyMessage parseFromData:_allFieldsData];
  self.unknownFields = _emptyMessage.unknownFields;
}
...
- (NSData *)getBizarroData
{
  ...
  for(NSNumber *key in _unknownFields.fields) {
    PBField *field = _unknownFields.fields[key];
...
- (void)testSerialize
{
  ...
  // again.
  NSData *data = [_emptyMessage data];
  STAssertEqualObjects(_allFieldsData, data, @"");
}

- (void)testCopyFrom
{
  TestEmptyMessage *message = [[[TestEmptyMessage builder] mergeFrom:_emptyMessage] build];
  STAssertEqualObjects(_emptyMessage.data, message.data, @"");
}

- (void)testMergeFrom
{
  PBUnknownFieldSet *set1 = [[[[PBUnknownFieldSet builder]
    addField:[[PBMutableField field] addVarint:2] forNumber:2]
    addField:[[PBMutableField field] addVarint:4] forNumber:3] build];

  PBUnknownFieldSet *set2 = [[[[PBUnknownFieldSet builder]
    addField:[[PBMutableField field] addVarint:1] forNumber:1]
    addField:[[PBMutableField field] addVarint:3] forNumber:3] build];

  PBUnknownFieldSet *set3 = [[[[PBUnknownFieldSet builder]
    addField:[[PBMutableField field] addVarint:1] forNumber:1]
    addField:[[PBMutableField field] addVarint:4] forNumber:3] build];

  PBUnknownFieldSet *set4 = [[[[PBUnknownFieldSet builder]
    addField:[[PBMutableField field] addVarint:2] forNumber:2]
    addField:[[PBMutableField field] addVarint:3] forNumber:3] build];
...
- (void)testClear
{
  PBUnknownFieldSet *fields = [[[[PBUnknownFieldSet builder] mergeUnknownFields:_unknownFields] clear] build];
  STAssertEquals(fields.fields.count, (NSUInteger)0, @"");
}

- (void)testClearMessage
{
  TestEmptyMessage *message = [[[[TestEmptyMessage builder] mergeFrom:_emptyMessage] clear] build];
...
- (void)testParseKnownAndUnknown
{
  ...
  PBUnknownFieldSet *fields = [[[PBUnknownFieldSet builderWithUnknownFields:_unknownFields]
    addField:[[PBMutableField field] addVarint:654321]
...
- (void)testUnknownExtensions
{
  ...
  TestEmptyMessageWithExtensions *message = [TestEmptyMessageWithExtensions parseFromData:_allFieldsData];

  STAssertTrue(_unknownFields.fields.count == message.unknownFields.fields.count, @"");
  STAssertEqualObjects(_allFieldsData, message.data, @"");
}
...
{
  NSData *data = [[[[PBUnknownFieldSet builder] addField:[[PBMutableField field] addVarint:0x7FFFFFFFFFFFFFFFL] forNumber:1] build] data];
